...
export async function retrieve(
  ...
  try {
    ...
    if (!listed.tools.some((t) => t.name === "groq_query"))
      throw new Error("CONTEXT_DATASET_MODE_REQUIRED");
    ...
    return {
      ...
      mode: "sanity-groq",
      notice:
        "Context 连接或查询未通过；已明确回退到 Sanity Content Lake。请运行 npm run verify:sanity 检查配置。",
    };
  } finally {
